...
3. **Attribute.** Connect it to the affected application/component and its owner (`02`).
4. **Prioritize.** Assign a priority tier and target window (`01`).
...
- **Every open finding has a named owner and a target date.** No ownerless or dateless items.
