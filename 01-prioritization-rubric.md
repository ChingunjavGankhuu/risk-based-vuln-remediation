...
## Impact modifiers — Operational Importance and Data Sensitivity

Operational importance — how critical the affected application or service is to operations: how much harm would result if it were disrupted or compromised. A user-facing application that many people depend on, or a core service other systems rely on, carries high operational importance and warrants escalation; a peripheral or lightly-used system may be de-escalated.

Data sensitivity — whether the affected application handles sensitive data, such as personally identifiable information (PII), financial, health, or confidential business data. Handling sensitive data warrants escalation, because exploitation causes greater harm; little or none may be de-escalated. Where your organization has a data classification standard, use it to define what counts as sensitive.

Higher operational importance or data sensitivity adjusts priority upward; lower may adjust it downward. These weigh the consequence of exploitation — they do not replace the exploitability assessment that sets the base priority.
